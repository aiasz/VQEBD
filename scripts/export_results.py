...
def populate_benchmark_runs(db: Database) -> int:
    ...
    seeded_count = 0
    res_exact = run_vqe(VQEConfig(molecule=mol, backend="qiskit_statevector", optimizer=OptimizerSpec(method="SLSQP")))
    ...
    for backend, opt in [
        ...
        cfg = VQEConfig(molecule=mol, backend=backend, optimizer=OptimizerSpec(method=opt, maxiter=100))  # type: ignore[arg-type]
    ...
    cfg_shot = VQEConfig(molecule=mol, backend="qiskit_aer_shot", optimizer=OptimizerSpec(method="COBYLA", maxiter=50))
    ...
    cfg_noisy = VQEConfig(molecule=mol, backend="qiskit_aer_noisy", optimizer=OptimizerSpec(method="COBYLA", maxiter=50))
    ...
    for extrap in ("richardson", "exponential"):
        cfg_zne = VQEConfig(
            ...
            mitigation=MitigationSpec(strategy="zne_local", scale_factors=(1, 3, 5), extrapolator=extrap),  # type: ignore[arg-type]
        )
    ...
    # 5. L5 Valódi IBM Heron hardveres mérés betöltése (data/raw vagy docs/figures/data)
    hw_candidates = [
    ...
    if hw_json is not None:
        ...
        if db.get_result(run_id_hw) is None:
            cfg_hw = VQEConfig(molecule=mol, backend="ibm_qpu", optimizer=OptimizerSpec(method="COBYLA"))
            ...
            hw_record = {
                ...
                "error_vs_exact_diag_ha": hw_raw.get("errors_ha", {}).get("error_vs_l1_ha", -0.003963),
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--db", default=DEFAULT_DB_PATH, type=Path, help="SQLite adatbázis útvonala")
    parser.add_argument("--out-csv", default=Path("data/exports/results_v1.csv"), type=Path, help="Cél CSV fájl")
    parser.add_argument("--out-json", default=Path("data/exports/results_v1.json"), type=Path, help="Cél JSON fájl")
